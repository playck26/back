...
| **A política de CORS é uma LISTA EXATA, e produção a executa** (`opcoesDeCors` em `common/validation/cors.ts`; instalada por `criarAppDeProducao`) | **Três provas em camadas, e cada uma alcança o que a outra não alcança.** (1) **Forma:** `cors.spec.ts` afirma o **conjunto de chaves** `{origin, credentials, maxAge}` — não uma propriedade —, que `origin` é `Array` e nunca predicado, e que a dedução é `trim` → vazios → dedupe. Derruba curinga, `RegExp` e callback **sem depender de qual origem alguém imaginou**. (2) **Efeito:** `cors-preflight.e2e-spec.ts`, com as três origens normativas e **quatro** negativos nomeados — externa, irmão não listado, **sufixo colado** e `Origin: null` — mais um controle sem `Origin`. (3) **Ligação:** o espião em `bootstrap-de-producao.spec.ts` conta **uma** chamada com o objeto inteiro, um gate de **conjunto** afirma que só `configurar-app.ts` chama `.enableCors(` em produção, e a `AC-002` da SPEC-071 fecha a sequência do `bootstrap()`. **Limite que fica:** a camada de injeção de dependência (`APP_GUARD` e afins) não aparece em nenhuma das três — `LIM-071f`, com cinco gatilhos nomeados |
...
| **UUID de rota e de corpo chegam ao handler na grafia do banco** (`UuidCanonicoPipe` em todo `@Param` de id; `@UuidNoCorpo` em todo DTO de entrada) | **Quatro camadas, e o limite de cada uma declarado ao lado dela — foi anunciar cobertura que uma camada não tinha que produziu sete rodadas de achado.** (1) **Declaração:** os dois gates leem metadado de runtime, exigem **identidade exata** do pipe com nada depois dele na cadeia, e `@Transform` que roda em `PLAIN_TO_CLASS` (um `{ toPlainOnly: true }` satisfazia a versão anterior sem normalizar nada). (2) **Descoberta:** controllers pelo grafo de módulos, métodos pelo **`MetadataScanner` do próprio Nest** — `getOwnPropertyNames` via 56 parâmetros onde o Nest registra 60, perdendo o que `CourtSportsController`/`CourtCategoriesController` herdam de `CatalogoController`; DTOs pelos `design:paramtypes` das rotas. (3) **Resultado:** `fronteira-do-uuid.http.spec.ts`, por HTTP. (4) **Bootstrap:** `bootstrap-de-producao.spec.ts` executa `criarAppDeProducao`, e **desde a SPEC-071 o `bootstrap.spec.ts` executa o `bootstrap()` inteiro**, com dois observadores complementares — log dinâmico das chamadas que atravessam `app` e `SwaggerModule`, e allowlist AST do arquivo `bootstrap.ts`. **Limite que fica:** guard roda antes de pipe e precisa de prova própria (só o `TenantGuard` lê `params` hoje). O *"nada impede o `main.ts` mexer no app depois de recebê-lo"*, que ficou aqui desde a 7ª rodada da SPEC-001, **caiu**: o `main.ts` tem duas instruções, afirmadas por AST. A fresta remanescente é outra, e está declarada — a camada de DI (`APP_GUARD` e afins) e o nível superior dos módulos que o `bootstrap.ts` importa |
