-- SPEC-032:TASK-001, fase EXPAND — rastreabilidade de acao administrativa.
--
-- ## POR QUE DUAS MIGRATIONS, E NAO UMA
--
-- O deploy do DigitalOcean **aplica migration sozinho** no push para `main`
-- (confirmado pelo Israel em 2026-09-02), e durante o deploy o conteiner
-- ANTIGO continua servindo trafego. Se a trigger
-- `ocupacao_cancelada_exige_evento` entrasse junto, haveria uma janela em que
-- ela existe e o codigo velho cancela sem evento — **todo cancelamento de
-- cliente virando 500**.
--
-- Esta fase e SEGURA de aplicar antes do codigo: cria tabelas e colunas que
-- ninguem ainda usa, e triggers que so protegem tabelas novas e vazias.
-- Nenhuma linha de comportamento existente muda.
--
-- A trigger que MUDA comportamento vai na fase `contract`, aplicada **depois**
-- que o codigo que grava eventos estiver no ar. Mesmo padrao de
-- `chamadas_expand/contract`, `spec019_turma_encontros_expand/contract` e
-- `spec020_catalogos_de_quadra_expand/contract`.
--
